        ...
        <div class="col-lg-8">
            <div class="card">
                <h4 class="card-header">
                    Edit {{ $data->checklist_detail->title }}
                </h4>
                <form
                    action="{{ route('checklis-detail.update', ['checklist' => $data->checklist->id, 'checklist_detail' => $data->checklist_detail->id]) }}"
                    method="post">
                    @csrf
                    @method('put')
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="title" value="{{ old('title', $data->checklist_detail->title) }}"
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="datetime-local" name="start_date"
                                        value="{{ old('start_date', $data->checklist_detail->start_date) }}"
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="datetime-local" name="end_date"
                                        value="{{ old('end_date', $data->checklist_detail->end_date) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <select class="single-select @error('member') is-invalid @enderror" name="pic"
                                    ...
                                    @foreach ($data->users as $item)
                                        <option @selected($item->id == $data->checklist_detail->user_id) value="{{ $item->id }}">
                                            {{ $item->email }}</option>
                                    @endforeach
                                </select>
                                @error('member')
                    ...
                    <div class="card-footer text-end">
                        <a href="{{ route('checklist-detail.index', ['checklist' => $data->checklist->id]) }}"
                            class="btn btn-secondary">Back to Checklist</a>
