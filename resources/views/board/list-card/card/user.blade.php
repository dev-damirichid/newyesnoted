...
@section('content')
    <div class="page-breadcrumb align-items-center mb-3">
        <div class="breadcrumb-title pe-3">#User Card {{ $data->board->title }}</div>
        <div class="ps-3">
        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                            ...
                            <div class="d-flex flex-column me-2 align-items-center">
                                ...
                                <div class="">
                                    <form class="deleteUserBoard"
                                        action="{{ route('card.destroy.user', ['board' => $data->board->slug, 'list_card' => $data->list_card, 'card' => $data->card->id, 'id' => $item->id]) }}"
                                        method="post">
                                        @method('delete')
                                        @csrf
                    ...
                    <form
                        action="{{ route('card.store.user', ['board' => $data->board->slug, 'list_card' => $data->list_card, 'card' => $data->card->id]) }}"
                        method="post">
                        @csrf
                        <input type="hidden" name="card" value="{{ $data->card->id }}">
                        <div class="mb-3">
